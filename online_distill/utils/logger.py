...
def setup_logger(
    ...
    *,
    ...
    log_func_name: bool = False,
):
    """
    setup logger that is compatiable with distributed system
    .. note::
        Only rank 0 will be allowed to log to console.
    Args:
        ...
        log_func_name: if True, include function name in the log format. We will enable it automatically when level is
            "debug".
    """
    ...
    # basic formatter
    if log_func_name:
        fmt = f"[RANK{rank}] %(asctime)s [%(levelname)s] [%(name)s.%(funcName)s:%(lineno)d] %(message)s"
    else:
        fmt = f"[RANK{rank}] %(asctime)s [%(levelname)s] [%(name)s:%(lineno)d] %(message)s"
